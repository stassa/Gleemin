...
*/	
	...
	destroy(X,Y) --> (['Destroy'];[destroy]), target(X,Z),[or], target(Y,A),
		{ permanent_Type(Z), permanent_Type(A) }.
...
%	target(X,Y) --> [target, Y], 	% Swi, 25/06/11
	/* target(X,Y) --> ([target];['Target']), [Y],
		{zone('Battlefield', Cards),
		 X = object(Name -_Id,_State),
		member(X, Cards),
...
%	target(X) --> [target, Y],
/*	target(X) --> ([target];['Target']), [Y], % Swi, 29/06/11, but this is useful
		{zone('Battlefield', Cards),
		 X = object(Name -_Id,_State),
		member(X, Cards),
		check_type(Name, _, [Y], _)}. */
